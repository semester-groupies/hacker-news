...
node { // The start of our build.
    ...
    try { // Checking what type of branch is currently building
        ...
        if  (env.BRANCH_NAME != 'master') { // it's a ready branch
            ...
            //test() // No testing (yet) for the front end..
            merge_and_push()
            clean_up()
...
def merge_and_push() {
    ...
    // Credentials are created in the jenkins/credentials/global/add_credentials
        withCredentials([usernamePassword(credentialsId: 'git-credentials', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
            ...
            //sh('git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/semester-groupies/hacker-news-frontend.git --tags')
            sh('git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/semester-groupies/hacker-news-frontend.git master') // Pushes to remote origin master branch
            sh('git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/semester-groupies/hacker-news-frontend.git --delete ' + env.BRANCH_NAME) // Deletes the ready/<name> branch.
        }
    }
}
...
def dockerBuild() {
    stage('Build image') {
        ...
        sh 'docker build -t favl/hacker-news-clone-frontend .'
    }
}
...
def dockerPush () {
    stage('Push image') {
        ...
        withCredentials([usernamePassword(credentialsId: 'docker-hub-credentials', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME')]) {
            ...
            sh('docker push favl/hacker-news-clone-frontend:latest')
            sh('docker logout')
        }
    }
}
